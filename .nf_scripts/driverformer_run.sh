...
# --------- 입력/파라미터 (환경변수로 전달됨) ----------
CLS="${NXF_CLS:-CLS}"              # 스테이징된 파일명(실제 파일은 현재 PWD에 존재)
FEAT="${NXF_FEAT:-FEAT}"
MUTS="${NXF_MUTS:-MUTS}"
REQS="${NXF_REQS:-REQS}"
...
SEGLEN="${NXF_SEGLEN:-}"                 # "10 50 100" 혹은 빈값
...
# requirements.txt 설치(있을 때만, torch/nvidia 계열 제외)
if [ -f "${REQS}" ] && [ "$(basename "${REQS}")" = "requirements.txt" ]; then
  echo "[SETUP] Installing requirements.txt (filtered)"
  grep -viE '^(torch|torchvision|torchaudio|pytorch-triton|triton|nvidia-|cuda|cudnn|cublas|cusparse|cusolver|nccl|cudatoolkit)' \
...
# wheels 폴백
if [ -d wheels ]; then
  echo "[SETUP] Installing from local wheels"
...
try:
  import driverformer;print("driverformer OK:",getattr(driverformer,"__version__","NA"))
except Exception:
  print("driverformer import FAIL");traceback.print_exc()
PY

# ---- 공통 인자 구성 (segment_lengths는 조건부 추가) ----
ARGS=(
  --cls-file            "${CLS}"
  --feat-file           "${FEAT}"
  --mutations-file      "${MUTS}"
  ...
  --huber-factor        "${HUBER}"
  --cutmix-p            "${CUTMIX}"
  --num-data-workers    "${NWORKERS}"
  --torch-threads       "${TTHREADS}"
  --len-alpha           "${LEN_ALPHA}"
  --res-beta            "${RES_BETA}"
  --label-roll-width    "${LABEL_W}"
  --pipeline-out-dir         "${P_OUT}"
  --pipeline-chunk-size      "${P_CHUNK}"
  --pipeline-chunk-overlap   "${P_OVERLAP}"
  --pipeline-min-distance    "${P_MINDIST}"
  --pipeline-max-distance    "${P_MAXDIST}"
  --pipeline-sample-frac     "${P_FRAC}"
  --pipeline-gmm-k           "${P_GMMK}"
  --pipeline-beta            "${P_BETA}"
  --pipeline-gamma          "${P_GAMMA}"
  --pipeline-seed            "${P_SEED}"
  --pipeline-dp-gap-bp       "${P_GAP}"
  --postsel-fdr-method       "${FDR_METHOD}"
  --postsel-bootstrap        "${BOOT}"
  --postsel-lambda-start     "${LAMBDA_S}"
  --postsel-lambda-end       "${LAMBDA_E}"
  --postsel-lambda-step      "${LAMBDA_T}"
  --postsel-pi0-floor        "${PI0_F}"
  --postsel-pi0-ceil         "${PI0_C}"
)
# segment_lengths 파싱(문자열이면 정규화)
if [ -n "${SEGLEN}" ]; then
  S=$(echo "${SEGLEN}" | tr ',' ' ' | sed -e 's/^ *//; s/ *$//' -e 's/  \+/ /g' -e 's/^"//; s/"$//')
  # 숫자 토큰만 추출
  S=$(for t in $S; do case "$t" in (''|*[!0-9]*) ;; (*) printf '%s ' "$t";; esac; done | sed 's/ *$//')
  if [ -n "${S}" ]; then
    ARGS+=( --segment-lengths ${S} )
  fi
fi
...
print(1 if importlib.util.find_spec("driverformer") else 0)
PY
then
