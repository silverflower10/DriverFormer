// main.nf — external bash runner (no triple quotes)
nextflow.enable.dsl = 2

// minimal params (그 외는 runner가 NXF_*로 받음)
...
process DRIVERFORMER_RUN {
  ...
  // <<< NO TRIPLE QUOTES >>> just one line
  script:
    "NXF_CLS='!{CLS}' NXF_FEAT='!{FEAT}' NXF_MUTS='!{MUTS}' " +
    "NXF_REQS='!{REQS}' NXF_WHEELS='!{WHEELS_DIR}' " +
    "NXF_OUT_DIR='!{params.out_dir}' " +
    "NXF_LR='!{params.lr}' NXF_BATCH='!{params.batch_size}' NXF_EPOCHS='!{params.epochs}' " +
    "NXF_SEED='!{params.seed}' NXF_DMODEL='!{params.d_model}' NXF_NHEAD='!{params.nhead}' NXF_NLAYERS='!{params.num_layers}' " +
    "NXF_DFF='!{params.dim_feedforward}' NXF_DROPOUT='!{params.dropout}' NXF_MAXLEN='!{params.max_seq_len}' " +
    ...
    "bash ${projectDir}/.nf_scripts/driverformer_run.sh"
}
