...
#%%
# JSON 설정 파일 경로
config_path = "/home/silverflo/BORI/experiment/configs/model_config1_11_20.json"
...
# 모델 실행
output_path = "/home/silverflo/BORI/experiment/results/basic_params1_11_20_op.pkl"
set_seed(config["SEED"]) 
results = run_BORI(config, data, output_path)

#%%
results = pd.read_pickle("/home/silverflo/BORI/experiment/results/basic_params1_11_20_op.pkl")
...
#%%
# 실제 counts와 expected_count를 비교하는 scatter plot (행, 열 바꿈)
plt.figure(figsize=(10, 6))
plt.scatter(results['count'], results['mean_expected_count'], alpha=0.3, s=8)
plt.plot([results['count'].min(), results['count'].max()], 
         [results['count'].min(), results['count'].max()], 
         color='red', linestyle='--')  # y=x 라인을 추가하여 비교 기준 제공
plt.title('Comparison of Counts and Expected Counts')
plt.xlabel('Counts')
...
# 실제 count와 예상 count 값에 노이즈 추가
noise_actual = np.random.normal(0, 0.5, size=len(results['count']))  # 실제 count에 대한 노이즈
noise_expected = np.random.normal(0, 0.5, size=len(results['mean_expected_count']))  # 예상 count에 대한 노이즈

noisy_actual_counts = sorted(results['count'] + noise_actual)
noisy_expected_counts = sorted(results['mean_expected_count'] + noise_expected)
