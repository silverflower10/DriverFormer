...
def find_low_probability_significant(df, posterior_col="posterior_probability", alpha=0.05):
    """
    Identify significantly low posterior probabilities using Bayesian FDR.
    
    Args:
        df (pd.DataFrame): Input DataFrame containing posterior probabilities.
        posterior_col (str): Column name with posterior probabilities.
        alpha (float): Desired FDR threshold.
    
    Returns:
        pd.DataFrame: DataFrame with FDR-adjusted results.
    """
    # Sort by posterior probabilities in ascending order (low probabilities first)
    df = df.sort_values(by=posterior_col, ascending=True).reset_index(drop=True)
    df["rank"] = df.index + 1  # Rank starts from 1

    # Compute adjusted FDR threshold for each rank
    n = len(df)
    df["fdr_threshold"] = alpha * df["rank"] / n

    # Mark significant results where posterior probability <= FDR threshold
    df["significant_low_probability"] = df[posterior_col] <= df["fdr_threshold"]

    return df

# Apply Bayesian FDR to find low probability significance
low_prob_results = find_low_probability_significant(df, posterior_col="posterior_probability", alpha=0.05)

# Display results
print(low_prob_results)
#%%
results = low_prob_results
# Manhattan Plot with Highlight for Low Posterior Probability
plt.figure(figsize=(25, 10))
...
# 데이터 분리: `significant_low_probability`가 True인 경우
highlight_data = results[results['significant_low_probability']]
normal_data = results[~results['significant_low_probability']]
...
plt.scatter(
    ...
    s=5,
    alpha=0.7,
    label='Significant (Low Probability)'
)
...
# 축과 제목
plt.title('Manhattan Plot with Highlighted Significant Probabilities', fontsize=16)
plt.xlabel('Chromosomes', fontsize=12)
plt.ylabel('-log10(Posterior Probability)', fontsize=12)
plt.legend(loc='upper right', fontsize=10)
plt.grid(axis='y', linestyle='--', alpha=0.5)
