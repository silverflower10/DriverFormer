...
#%%
# JSON 설정 파일 경로
config_path = "/home/silverflo/BORI/experiment/configs/model_config2_11_21.json"
...
# 모델 실행
output_path = "/home/silverflo/BORI/experiment/results/config2_11_21_op.pkl"
set_seed(config["SEED"]) 
results = run_BORI(config, data, output_path,  num_samples = 1000)

#%%
results = pd.read_pickle("/home/silverflo/BORI/experiment/results/config2_11_21_op.pkl")
...
plt.figure(figsize=(10, 6))
plt.scatter(results['actual_counts'], results['expected_count'], alpha=0.3, s=8)
plt.plot([results['actual_counts'].min(), results['expected_count'].max()], 
         [results['actual_counts'].min(), results['expected_count'].max()], 
         color='red', linestyle='--')  # y=x 라인을 추가하여 비교 기준 제공
