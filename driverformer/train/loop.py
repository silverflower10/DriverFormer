...
_RATE_MIN  = 1e-9
_RATE_MAX  = 1e6  # 여유 상한
STRICT_BACKWARD = os.environ.get("DF_STRICT_BACKWARD", "0") == "1"  # anomaly 끄기 기본
...
def train_and_predict(args):
    ...
    if train_flag:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for _step, batch in enumerate(train_loader):
                ...
                for m in model_components.values():
                    for p in unwrap(m).parameters():
                        if p.grad is not None and not torch.isfinite(p.grad).all():
                            ...
                            _bad += 1
                if _bad:
                    print(f"[GRAD] sanitized { _bad } parameter grads (nan/inf → 0.0)", flush=True)
                ...
                if step_global % log_interval == 0:
                    ...
                    if DEBUG_NAN and 'lam_dbg' in locals():
                        ...
                        print(f"[Epoch {epoch} | Step {step_global}] loss={avg:.4f}  μ_mean={mu_mean:.4g}  y_mean={y_mean:.4g}  μ/y={ratio:.3f}  scale={scale:.3f}", flush=True)
                    else:
            ...
            if _logc_backup := locals().get("_logc_backup", None):
                with torch.no_grad():
                    unwrap(model_components["nhpp_head"]).log_c.copy_(_logc_backup)
