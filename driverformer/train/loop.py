...
* NaN 방지 가드(λ/y/Δt 안전화, soft clamp, 샘플러 확률 폴백, 평가/예측 경로 동일 적용)
"""
...
# --------------------------------------------------------------------------- #
@torch.no_grad()
def calibrate_log_c_huber_like_training(model_c,
                                        loader,
                                        device,
    ...
    for b in loader:
        ...
        lam      = _safe_rate_(model_c["nhpp_head"](
    ...
    def _hw(r, d):
        a = abs(r)
        return 1.0 if a <= d else d / (a + 1e-9)
    ...
    if (not math.isfinite(num)) or (not math.isfinite(den)) or num <= _EPS or den <= _EPS:
        print(f"[CAL-HUBER-TRAIN] skip (num={num:.3g}, den={den:.3g}) keep c={c_cur:.6g}", flush=True)
        return

    c_star = num / den
    if math.isfinite(c_star) and c_star > 0:
        ...
        print(f"[CAL-HUBER-TRAIN] delta={delta:.4g}  c_prev={c_cur:.6g}  c_new={c_star:.6g}  "
              f"ratio={c_star/(c_cur+_EPS):.4f}", flush=True)
    else:
        print(f"[CAL-HUBER-TRAIN] invalid c*: {c_star}", flush=True)
...
def train_and_predict(args):
    ...
    # ── CPU 스레드 수 제어(옵션) ────────────────────────────────────────────
    if getattr(args, "torch_threads", None) is not None and args.torch_threads > 0:
        try:
    ...
    # ===== mutations CSV로부터 1kb-bin 라벨 생성(하드 PASS) =====
    ...
    for c in c_map:
        ...
        n = len(lst)
        n_train = int(n * 0.9)
        train_segs.extend(lst[:n_train])
        val_segs.extend(lst[n_train:])
    ...
    model_components = dict(
        feature_embedder=feat_embedder,
    ...
    if train_flag:
        ...
        epochs_no_improve, step_global = 0, start_epoch * max(1, len(train_loader))
        ...
        def weighted_loss_one_batch(batch, return_lam=False):
            ...
            _assert_finite("lam", lam)
            _assert_finite("y_b", y_b)
            _assert_finite("len_b", len_b)
            ...
            for _step, batch in enumerate(train_loader):
                ...
                if step_global % log_interval == 0:
                    ...
                    if DEBUG_NAN:
                        ...
                        scale    = torch.exp(unwrap(nhpp_h).log_c).item()
                        ratio    = mu_mean / max(y_mean, _EPS)

                        print(f"[Epoch {epoch} | Step {step_global}] "
                              f"batch_avg_loss={avg:.4f}  μ_mean={mu_mean:.4g}  "
                              f"y_mean={y_mean:.4g}  μ/y={ratio:.3f}  scale={scale:.3f}",
                              flush=True)
                    else:
            ...
            def eval_loader(loader):
                ...
                with torch.no_grad():
                    for b in loader:
                        ...
                        for i, seg in enumerate(b["raw_segments"]):
                            L = seg["cls_array"].shape[0]
                            res[seg["global_idx"]] = float(
                                (y_b[i, :L] - mu_b[i, :L]).mean().detach().cpu().item()
                            )
                ...
                with torch.no_grad():
                    for b in loader:
                        ...
                        sum_log = (y_b * torch.log(lam)).sum(dim=1)       # (B,)
                        integ   = (lam * len_b).sum(dim=1)                 # (B,)
                        neg_ll  = -(sum_log - integ)                       # (B,)
                        seg_len = (len_b.sum(dim=1) + _EPS)                # (B,)
            ...
            def compute_segment_residual():
                ...
                with torch.no_grad():
                    for b in train_loader_infer:
                        ...
                        if args.label_roll and args.label_roll_width > 1:
                            lam, y_b, _ = rolling_sum_nhpp(lam, y_b, len_b, width=args.label_roll_width)
        ...
        plt.savefig(os.path.join(out_dir, "train_val_loss_per_epoch.png"))
        plt.close()
    ...
    def predict_and_save(loader, name):
        ...
        with torch.no_grad():
            for b in loader:
                ...
                # 저장용 lam은 롤링 스무딩이 켜졌다면 동일 스무딩을 반영
                lam_save = lam
                if args.label_roll and args.label_roll_width > 1:
                    lam_r, _y_ignore, _dt_ignore = rolling_sum_nhpp(lam, y_b, len_b, width=args.label_roll_width)
                    lam_save = lam_r

                lam_np_raw = lam.detach().cpu().numpy()       # 로그우도 계산용(원본)
                ...
                for i, seg in enumerate(b["raw_segments"]):
                    ...
                    lam_raw_i_safe = np.clip(lam_raw_i, _RATE_MIN, _RATE_MAX)
                    ...
                    for j in range(L_eff):
                        rows.append(dict(
                            ...
                            lam_pred=float(lam_i[j]),     # 스무딩 반영된 1kb당 rate
                            obs_count=float(y_i[j]),
                            bin_loglike=float(llb_i[j]),
                        ))
                        tot_ll  += float(llb_i[j])
                        tot_bin += 1
